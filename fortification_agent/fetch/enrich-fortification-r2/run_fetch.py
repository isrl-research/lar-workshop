#!/usr/bin/env python3
"""
Task: Enrich fortification substances with nutrient metadata — Round 2
Model: gemini-2.0-flash
Temperature: 0
Batch size: 20
Input: input_substances.txt (20 substances, IDs 59-78)
Created: 2026-03-14
Note: IDs continue from round 1 (01-58). Output files are r2-specific.
      After run, append output_formatted_r2.txt to ../enrich-fortification/output_formatted.txt
      and fold rows into fortification_agent_working.csv in the next fortification session.
"""
...
RAW_OUT      = "./output_raw_r2.txt"
FMT_OUT      = "./output_formatted_r2.txt"
...
def print_statistics(formatted_path, expected_total):
    ...
    total = len(lines)
    print(f"\n{'='*60}")
    print(f"OUTPUT STATISTICS (Round 2)")
    print(f"{'='*60}")
...
def main():
    ...
    print("Next steps:")
    print("  1. Review output_formatted_r2.txt for UNSURE / ?-prefix / PARSE_ERROR flags")
    print("  2. Append output_formatted_r2.txt to ../enrich-fortification/output_formatted.txt")
    print("  3. Fold rows into fortification_agent_working.csv (next fortification session)")
    print("  4. Handle dedup: cupric sulphate vs copper sulphate (IDs 67/68)")
    print("  5. Handle IS_GROUP flag for chloride (ID 60)")
